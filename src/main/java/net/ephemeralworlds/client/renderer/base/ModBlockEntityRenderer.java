...
public abstract class ModBlockEntityRenderer<T extends BlockEntity> implements BlockEntityRenderer<T> {
    ...
    public ModBlockEntityRenderer(BlockEntityRenderDispatcher ctx) {
    }
...
//
        GlStateManager._blendFunc(GlStateManager.DstFactor.SRC_ALPHA.value, GlStateManager.DstFactor.ONE_MINUS_DST_ALPHA.value);
        Tessellator tess = Tessellator.getInstance();
        BufferBuilder builder = tess.getBuffer();
//
//        GlStateManager.(renderX+.5, renderY+.5, renderZ+.5);
//        todo block end
...
//
//        builder.vertex(x1, y1, 0).texture(u0, v0).color(col.getRed() / 255F, col.getGreen() / 255F, col.getBlue() / 255F, alpha).light(0).normal(normalX, normalY, normalZ).next();
//        builder.vertex(x2, y1, 0).texture(u1, v0).color(col.getRed() / 255F, col.getGreen() / 255F, col.getBlue() / 255F, alpha).light(1).normal(normalX, normalY, normalZ).next();
//        builder.vertex(x2, y2, 0).texture(u1, v1).color(col.getRed() / 255F, col.getGreen() / 255F, col.getBlue() / 255F, alpha).light(2).normal(normalX, normalY, normalZ).next();
//        builder.vertex(x1, y2, 0).texture(u0, v1).color(col.getRed() / 255F, col.getGreen() / 255F, col.getBlue() / 255F, alpha).light(3).normal(normalX, normalY, normalZ).next();
//
//        tess.draw();
//
//
